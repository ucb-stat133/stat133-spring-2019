---
title: "Lab 5: Command Line Basics"
...
If you are using __git-bash__ (i.e. your OS is Windows) you don't have the `man` 
...
The second part of the lab involves downloading the __Abalone Data Set__ 
...
Let's begin by exporting the data `mtcars` to a CSV file, __executing commands from the console__. Suppose that we want to export `mtcars` inside the directory `exports`. The first thing to do is to find out what is the _working directory_ that the R console is paying attention to.
...
The first call to `sink()` opens a connection to the specified file, and then 
...
Here's how to save a simple scatterpot of `mpg` and 
`disp` in png format to the folder `exports/`:
